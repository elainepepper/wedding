...
function GuestModal({ guest, households, close, act }: { guest: Guest | "new"; households: Household[]; close: () => void; act: (payload: Record<string, unknown>, success: string) => Promise<unknown> }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    setSaving(true);
    try { await act({ action: isNew ? "addGuest" : "editGuest", guestId: current?.id, ...form, householdId: form.householdId ? Number(form.householdId) : null }, isNew ? "Guest added" : "Guest updated"); close();  } finally { setSaving(false); }
  };
