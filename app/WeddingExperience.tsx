...
export function WeddingExperience({ invitationToken, previewMode = false }: { invitationToken?: string; previewMode?: boolean }) {
  ...
  const [rsvp, setRsvp] = useState<RsvpState>(() => previewMode ? { ...initialRsvp, guestName: "dear guest", phoneNumber: "12345678", attendance: "yes", flyingIn: false } : initialRsvp);
  const [inviteData, setInviteData] = useState<InviteData | null>(null);
  const [guestResponses, setGuestResponses] = useState<GuestResponse[]>(() => previewMode ? [{ id: -1, name: "Your name", rsvpStatus: "Confirmed", ceremonyAttending: true, receptionAttending: true, afterPartyAttending: "Pending", mealSelection: "Salmon", dietaryRequirements: "", allergies: "", accessibility: "", transportRequired: false, accommodationRequired: false, travelArrival: "", travelDeparture: "", accommodationName: "", songRequest: "", wishes: "" }] : []);
  ...
  const sectionIds = useMemo(() => {
    ...
    if (!personalised || rsvp.attendance === "yes") ids.push("dress");
  ...
  }, [personalised, rsvp.attendance, submitted, hiddenScenes, customAfter]);
