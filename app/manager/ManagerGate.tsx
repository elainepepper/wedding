...
export function ManagerGate() {
  ...
  useEffect(() => {
    if (firebaseClientConfigurationError) {
      setLoading(false);
      return;
    }
  ...
  if (firebaseClientConfigurationError) return <main className="manager-login"><section className="manager-config" role="alert"><p>Elaine &amp; Haykal</p><h1>Guest Manager</h1><span>This copy still needs its Firebase web settings. Add the values listed in <b>.env.example</b> to <b>.env.local</b>, then restart the site.</span></section></main>;
